...
(() => {
	...
	if (yyyymmElement !== null && dayElement !== null && datepickerElement !== null) {
		...
		datepickerElement.addEventListener('change', (ev) => {
			let value = ev.target.value !== '' ? ev.target.value : ev.target.value;
			if (value === '') {
	...
	if (optionAreaElement !== null) {
		console.log('【検索画面】交通手段の初期設定');

		const courseSetting = supportGMgetValue ? GM_getValue('COURSE_SETTING', COURSE_SETTING) : COURSE_SETTING;
		for (const courseName in courseSetting) {
